...
class FilterService:
    ...
    def job_setup(self) -> tuple[RootJob, DatasetObject]:
        ...
        for job_name_key in JOB_SET.keys():
            ...
            logging.info(f'добавлена новая JOB, {job_name_key}')
            logging.info(f'описание JOB: {new_job.job_description()}\n')
    ...
    def filter_chain(self) -> None:
        ...
        for dataset, filename in self.datasets:
            ...
            logging.info(f'выполняется работа над датасетом: {dataset_object}')
            root.handle()
            dataset_object.save_dataset()
            logging.info(f'{filename} успешно проведен через все JOB и записан в {OUTPUT_PATH}\n')
